...
if (!preg_match("/^[0-9]*$/",$ph)) 
 ...
 if (empty($_POST["rank"])) {
  ...
  if (empty($_POST["f_name"])) {
   ...
   if (empty($_POST["b_no"])) {
     $b_noErr = "batch no is required";
   } else {
     $b_no = test_input($_POST["b_no"]);
     // check if name only contains letters and whitespace
     if (!preg_match("/^[0-9]{1,2}$/",$b_no)) {
       $b_noErr = "invalid batch no"; 
     }
   }
...
$sql = "insert into counsellor.student values('$name',$dob,'$sex',$yoa,'$usn',$batch,'$e_id',$ph,'$address','$mode',$rank,'$f_name','$f_id',$f_no,'$m_name','$m_id',$m_no)";
